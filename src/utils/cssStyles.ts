...
export default function cssStyles(theme?: Theme) {
  return {
    ...
    bgGradient: (props?: BackgroundGradientProps) => {
      ...
      const startColor = props?.startColor || `${alpha('#000000', 0)} 60%`;
      const endColor = props?.endColor || `${alpha('#000000', 10)} 100%`; // '#000000 100%'
    
      return {
        background: `linear-gradient(-165deg, ${startColor}, ${endColor});`,
      };
    },
